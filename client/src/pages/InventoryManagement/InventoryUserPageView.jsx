...
export default function InventoryUserPageView() {
  ...
  const handleSearch = (e) => {
    ...
    if (searchQuery.trim() !== '') { // Check if searchQuery is not empty
      const filtered = userview.filter((elem) => {
        const nameMatch = elem.name.toLowerCase().includes(searchQuery.toLowerCase());
        const priceMatch = elem.price.toString().includes(searchQuery);
        const supplierMatch = elem.supplier.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return (
    <div className="bg-paleblue">
      ...
      <h1 className='pt-4 text-center text-4xl text-blue font-bold'>View Available Items</h1>
      <div> 
        <form className='px-10 py-2 pb-7 flex justify-end' onSubmit={handleSearch}>
          <div className='relative'>
            <input type='text' placeholder='Search for items' className=' text-sm bg-white border-2 border-light-blue rounded-md placeholder-gray focus:outline-none w-56 p-2 pl-10' onChange={(e) => setSearchQuery(e.target.value)} value={searchQuery} />
      ...
      {noResults ? ( // Render the message only when noResults state is true
        <div className="p-7 flex justify-center">
          <p className='text-xl text-center text-red-400 font-bold'>Sorry, we don't have that Item.We're actively working on getting the products you need.<br/><p className='text-green-400 uppercase'>Stay tuned for updates!</p></p>
        </div>
      ) : (
        <div className="p-7 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {searchResults.map((elem, index) => (
            <GasItem key={index} item={elem} />
          ))}
        </div>
      )}
