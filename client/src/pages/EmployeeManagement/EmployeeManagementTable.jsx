...
function EmployeeTable() {
    ...
    const fetchData = async () => {
        try {
            const fetchEmployee = await axios.get('http://localhost:3000/api/employee/read');
    ...
    const handleDeleteConfirmed = async () => {
        try {
            await axios.delete(`http://localhost:3000/api/employee/delete/${deleteId}`);
            setData(prevState => ({
                ...prevState,
        ...
        <table className="w-full border-2 border-blue">
            <thead>
                <tr className="bg-blue text-white text-left">
                    ...
                    <th className="border border-blue px-4 py-2">Gender</th>
            ...
            <tbody>
                {searchResults?.map((elem, index) => {
                    ...
                    <tr key={index} className="bg-paleblue">
                        ...
                        <td className="border-b-2 border-b-blue px-4 py-2">{elem.gender}</td>
