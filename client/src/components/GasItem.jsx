...
export default function GasItem({ item }) {
  ...
  <div className="border-t-indigo-400 border-light-blue flex flex-col gap-4 w-75% sm:w-[330] rounded-2xl border-2 pl-2 p-14">
    <div className="flex flex-col">
      <div className='pl-2 flex flex-col gap-2 w-full '>
        ...
        <p className='text-xl font-semibold text-slate-700'>{item.name},  <p className=' text-light-blue'>Units:{item.quantity}</p></p>
        <img className='h-300 w-300' src={item.imageUrl} alt = "Image of medicine"/>
        <p className='text-lg text-green-600 truncate text-'>{item.storageCondition}</p>
        <div className="flex items-center gap-1">
          ...
          <p className='text-lg text-red-600 truncate'>Rs.{item.price}</p>
